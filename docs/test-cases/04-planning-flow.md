...
> **模块：** PLAN · **用例数：** 50 · **版本：** v2.0 · **更新：** 2026-06-03
...
|----------|----------|--------|------|--------|----------|----------|----------|----------|
| TC-PLAN-001 | inject_user_memory 注入偏好 | P1 | 功能 | ⏳ | 用户 preferences 非空 | 1. 进入 graph 2. 查 state | — | memory 注入 prompt；不改变 step |
...
| TC-PLAN-004 | plan_destination 需用户确认 selected | P1 | 功能 | ⏳ | 多候选 | 1. 用户选「就北京」 | — | selected_destination=北京 |
...
| TC-PLAN-007 | plan_transport 介绍三种方式 | P0 | 功能 | ⏳ | 进入 transport | 1. 读首条助手消息 | — | 含 flight/train/driving 说明 |
| TC-PLAN-008 | 选择 flight 航班 | P0 | 功能 | ⏳ | transport 步 | 1. 输入 `飞机` | flight | selected_transport=flight；→ stay_food |
...
| TC-PLAN-015 | 餐饮三枚举 specialty/chain/local | P0 | 功能 | ⏳ | stay 步 | 1. `本地小吃` | local | selected_food_types 含 local |
| TC-PLAN-016 | 组合「经济酒店+本地小吃」 | P0 | 功能 | ⏳ | stay 步 | 1. 一句组合 | smoke 数据 | 两列表均有值；→ activities |
...
| TC-PLAN-019 | 活动五类 culture/nature/food_tour/shopping/family | P0 | 功能 | ⏳ | activities 步 | 1. `文化体验` | culture | selected_activity_types 含 culture |
...
| TC-PLAN-025 | 超预算警告 Plan B | P0 | 功能 | ⏳ | budget_max 低 | 1. 生成行程 | 穷游+高消费组合 | 助手含超预算警告或 Plan B 建议 |
...
| TC-PLAN-030 | FAST_LLM 仅 collect | P2 | 功能 | ⏳ | — | 1. collect 用 fast；plan 用 pro | — | 模型 tier 正确 |
| TC-PLAN-031 | nl_extract 交通映射 | P1 | 功能 | ✅ | — | 1. 测中英文映射 | 飞机→flight | 映射正确 |
| TC-PLAN-032 | explore_agent 目的地探索 | P2 | 功能 | ⏳ | 无明确 destination | 1. explore 推荐 | — | 返回候选列表 |
...
| TC-PLAN-048 | 重复进入同一步幂等 | P2 | 异常 | ❌ | 已有 selected_transport | 1. 再发交通句 | — | 不重复重置 |
