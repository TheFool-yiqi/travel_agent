...
> **模块：** CHAT / API · **用例数：** 55 · **版本：** v2.0 · **更新：** 2026-06-03
...
| TC-CHAT-014 | WebSocket 与 SSE 行为一致 | P1 | 功能 | ⏳ | 同会话同输入 | 1. 分别调用 2. 对比 | 相同 content | 最终 state 等价 |
| TC-CHAT-015 | WS 认证超时关闭 | P1 | 异常 | ❌ | 不发 auth | 1. 等 10s | — | 4401 认证超时 |
| TC-CHAT-016 | 并发两路 stream 同会话 | P2 | 异常 | ❌ | — | 1. 并行 POST 2 次 | — | 串行或锁；数据不 corrupt |
| TC-CHAT-017 | stream 中断客户端断开 | P2 | 异常 | ❌ | 流式中 | 1. 中途 abort | — | 服务端清理；可继续新消息 |
...
| TC-CHAT-020 | 首 token 延迟可接受 | P3 | 性能 | ❌ | — | 1. 测 TTFB | — | P95 < 目标值 |
| TC-CHAT-021 | history 消息顺序 | P1 | 功能 | ✅ | 多轮 | 1. GET history | — | 按 created_at 升序 |
| TC-CHAT-022 | history 含 step metadata | P2 | 功能 | ⏳ | 规划消息 | 1. 查 message extra | — | 可选含 step/trace |
